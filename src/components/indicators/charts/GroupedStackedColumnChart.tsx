...
function GroupedStackedColumnChart({
  ...
  useLayoutEffect(() => {
    ...
    // Build subtype → color index map (ordered by appearance in data)
    const subtypeColorIndex: Record<string, number> = {};
    const allSubtypes: { name: string; colorIndex: number }[] = [];
    let ci = 0;

    const seenProviders = new Set<string>();
    const providerSubtypeOrder: { provider: string; subtype: string }[] = [];

    for (const row of data) {
      if (!seenProviders.has(row.provider)) seenProviders.add(row.provider);
      providerSubtypeOrder.push({ provider: row.provider, subtype: row.realName });
    }
    ...
    // Hidden subtype state (mutated by legend clicks, no re-render needed)
    ...
    // Axis ranges: one range per outer category (provider)
    ...
    // Closing tick after the last column
    if (data.length > 0) {
    ...
    // Bottom container: label + scrollable legend
    ...
    legend.itemContainers.template.events.on("click", (e) => {
      ...
      seriesList.forEach((series) => {
        series.columns.each((col) => {
          ...
          if (ctx?.realName === subtype) {
            col.set("forceHidden", nowHidden);
          }
        });
      });
    });
